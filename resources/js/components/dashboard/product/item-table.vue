          ...
          <table class="table" id="printMe">
            <thead>
              <tr>
                ...
                <th style="width: 23%" scope="col">{{ $t("CATEGORIES") }}</th>
                <th style="width: 10%" scope="col">{{ $t("PRICE") }}</th>
                <th style="width: 10%" scope="col">{{ $t("PUBLISHED") }}</th>
            ...
            <tbody v-if="items.length">
              <tr v-for="(item, index) in items" :key="item.id">
                ...
                <td>
                  <span class="normal" v-if="item.brand">
                    {{
                      $i18n.locale == "ar"
                        ? item.brand.name_ar
                        : item.brand.name_en
  ...
  setup() {
    ...
    onMounted(() => {
      productClient.getBrands().then((res) => {
        ...
        _data.unshift({
          id: -1,
          name_ar: "اختر علامة تجارية",
          name_en: "Select Brand",
        });
        data.brands = _data;
      });
    });
    ...
    function getItems() {
      productClient
        .getItems(data.page, data.pageSize, data.brand, data.status, data.text)
        .then((response) => {
